  ...
  fluidPage(
    ...
    tags$head(tags$style(
      HTML(
        ...
        #id-label, #secret-label, #inputTerm-label, #inputxAxis-label, #inputyAxis-label, #numberofrecomm-label, #dataset-label{
        color: #FFFFFF;
        }
        '
      )
    ...
    useSweetAlert(),
    shiny::navbarPage(
      title = div("Spotify User Analysis", tags$script(
        HTML(
    ...
    console.log(header)"
      ...
      tabPanel(
        ...
        sidebarPanel(
          ...
          # txt1 sent to the server
          textInput("secret", "Client Secret: ", ""),
          # txt2 sent to the server
      ...
      tabPanel(
        ...
        sidebarPanel(
          ...
          fluidRow(
            img(src = "anime.gif", style = "display: block; margin-left: auto; margin-right: auto;")
          )
        ),
        ...
        mainPanel(
          fluidRow(column(
            4, tags$h3("Top 10 Most Listened Artists"), offset = 2
          ),
          column(
            4, tags$h3("Top 10 Most Listened Tracks"), offset = 2
          )),
          fluidRow(
            column(4, plotlyOutput("top5artistsTable", width = "125%"), offset = 1),
            column(4, plotlyOutput("top5tracksTable", width = "125%"), offset = 2)
          ),
          br(),
          fluidRow(column(
            4,
            tags$h3("How often does the artist appear in liked songs?"),
            offset = 2
          ),
          column(
            4, tags$h3("Top 10 Summarized Artists Song Duration"), offset = 2
          )),
          fluidRow(
            column(
              4,
              plotlyOutput("top10favouriteArtist", width = "125%"),
              offset = 1
            ),
            column(4, plotlyOutput("top10durationsong", width = "125%"), offset = 2)
          )
      ...
      tabPanel(
        ...
        mainPanel(
          tags$h3(
            "Let's look closer at statistics of every song that belongs to Liked Playlist"
          ),
          fluidRow(column(
            12, plotlyOutput("CategoryPlot", height = "125%")
          ))
        )
      ),
      tabPanel(
        "Recommendations",
        sidebarPanel(
          tags$h3("Choose artist from your Liked Songs to get recommendations:"),
        ...
        mainPanel(
          tags$h3(
            "Let's take a look at how the liked songs group together in Principal Component Analysis"
          ),
          fluidRow(column(
            12, plotlyOutput("plot_3dpca", width = "125%", height = "125%")
          )),
          fluidRow(column(5, tags$h3("Recommendations"), offset = 5)),
          fluidRow(column(
            6, plotlyOutput("recommend", height = "125%"), offset = 3
          ))
        )
      )
